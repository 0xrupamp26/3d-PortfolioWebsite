...
const AnimatedBackground = () => {
  ...
  // Map old key names from the Spline scene to new Web3 skill keys
  const SKILL_ALIAS: Record<string, string> = {
    // legacy web2 keys -> web3 keys
    typescript: "solidity",
    javascript: "rust",
    nextjs: "rust",
    react: "move",
  ...
  // Reverse map: from web3 skill keys back to legacy Spline object names
  const REVERSE_ALIAS: Record<string, string> = Object.entries(SKILL_ALIAS).reduce(
    (acc, [legacy, web3]) => {
      acc[web3] = legacy;
      return acc;
    },
  ...
  const handleMouseHover = (e: SplineEvent) => {
    if (!splineApp) return;
    const raw = (e.target.name || "").toLowerCase();
    if (selectedSkill?.name === raw) return;

    if (raw === "body" || raw === "platform") {
      ...
      if (!selectedSkill || selectedSkill.name !== raw) {
        const key = SKILLS[raw] ? raw : SKILL_ALIAS[raw];
        const skill = key ? (SKILLS as Record<string, Skill>)[key] : undefined;
  ...
  const getKeycapsAnimation = () => {
    ...
    const start = () => {
      ...
      Object.values(SKILLS)
        ...
        .forEach((skill, idx) => {
          const objectName = REVERSE_ALIAS[skill.name] || skill.name;
